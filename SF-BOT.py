...
class StreakBot:
    ...
    def get_user_data(self, user_id):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT streak_days, last_post_date, score FROM user_streaks WHERE user_id = ?",
            (user_id,),
        )
        result = c.fetchone()
        conn.close()
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        print(
                            f"Successfully downloaded image, size: {len(image_data)} bytes"
                        )
                        return image_data
                    else:
                        print(
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching, name="your streaks! Use /help"
        )
    )
...
async def on_message(message):
    ...
    print(
        f"Message received in channel {message.channel.id} ({message.channel.name})"
    )

    # Check if message is in an image channel
    if not streak_bot.is_image_channel(message.channel.id):
        print(
            f"Channel {message.channel.id} is NOT an image channel, ignoring images..."
        )
        return
    ...
    print(
        f"Channel {message.channel.id} IS an image channel, processing images..."
    )
    ...
    if message.attachments:
        ...
        if image_attachments:
            try:
                print(
                    f"Processing image from {message.author.display_name} in image channel {message.channel.name}..."
                )
                ...
                streak_days, new_score = (
                    streak_bot.update_user_streak_and_score(user_id, username)
                )
                print(
                    f"User streak: {streak_days} days, Score: {new_score} points"
                )
                ...
                file = discord.File(
                    io.BytesIO(image_data), filename="streak_image.png"
                )
...
async def sync(ctx):
    ...
    try:
        ...
        await ctx.send(
            f"✅ Synced {len(synced)} slash command(s)! They should be available immediately."
        )
        print(
            f"Manually synced {len(synced)} slash commands to {ctx.guild.name}"
        )
...
async def set_image(ctx):
    ...
    if channel_id not in streak_bot.image_channels:
        ...
        await ctx.send(
            f"✅ Set {ctx.channel.mention} as an image channel! Bot will now process images here."
        )
        print(
            f"Added channel {channel_id} to image channels. Current image channels: {streak_bot.image_channels}"
        )
    else:
...
async def list_image_channels(ctx):
    ...
    if streak_bot.image_channels:
        ...
        embed = discord.Embed(
            ...
            if channel_mentions
            else "No image channels set!",
            color=0x7289DA,
        )
        await ctx.send(embed=embed)
    else:
        await ctx.send(
            "❌ No image channels set! Use `!set_image_channel` to add one."
        )
...
async def debug_image_channels(ctx):
    ...
    embed.add_field(
        ...
        value=str(streak_bot.image_channels)
        if streak_bot.image_channels
        else "Empty list!",
        inline=False,
    )
...
async def remove_image(ctx):  # REMOVED @not_image_channel()
    ...
    if channel_id in streak_bot.image_channels:
        ...
        await ctx.send(
            f"✅ Removed {ctx.channel.mention} from image channels! Bot will no longer process images here."
        )
        print(
            f"Removed channel {channel_id} from image channels. Current image channels: {streak_bot.image_channels}"
        )
    else:
...
async def streak_slash(interaction: discord.Interaction):
    ...
    try:
        ...
        if user_data:
            ...
            embed.add_field(
                name="Current Streak",
                value=f"{streak_days} days {streak_emoji}",
                inline=False,
            )
            embed.add_field(name="Last Post", value=last_post_date, inline=True)
            embed.add_field(
                name="Total Score", value=f"{score} points", inline=True
            )

            last_post_datetime = datetime.datetime.strptime(
                last_post_date, "%Y-%m-%d"
            ).date()
        ...
        await interaction.response.send_message(
            "Error retrieving your streak information."
        )


@bot.tree.command(
    name="score", description="Check your current score and stats"
)
@not_image_channel()
async def score_slash(interaction: discord.Interaction):
    ...
    try:
        ...
        if user_data:
            ...
            embed.add_field(
                name="Next Milestone",
                value=f"{next_milestone} points ({points_needed} more)",
                inline=True,
            )
...
async def leaderboard_slash(interaction: discord.Interaction):
    ...
    try:
        ...
        if leaders:
            for i, (username, score, streak_days, last_post_date) in enumerate(
                leaders, 1
            ):
                medal = (
                    "🥇"
...
@bot.tree.command(
    name="streak_leaderboard", description="Show the top streak leaders"
)
@not_image_channel()
async def streak_leaderboard_slash(interaction: discord.Interaction):
    ...
    try:
        ...
        if leaders:
            for i, (username, streak_days, score, last_post_date) in enumerate(
                leaders, 1
            ):
                medal = (
                    "🥇"
            ...
            embed.description = (
                "No active streaks! Start a streak by posting an image!"
            )

        await interaction.response.send_message(embed=embed)
    except Exception as e:
        await interaction.response.send_message(
            "Error retrieving streak leaderboard."
        )
...
async def user_stats_slash(
    ...
    try:
        ...
        if user_data:
            ...
            embed.add_field(
                name="Streak Status", value=streak_status, inline=True
            )
...
@discord.app_commands.describe(
    user="The user to add points to", points="Number of points to add"
)
@not_image_channel()
async def add_score_slash(
    ...
    try:
        ...
        # Get current score
        c.execute(
            "SELECT score FROM user_streaks WHERE user_id = ?", (user.id,)
        )
        result = c.fetchone()
        ...
        await interaction.response.send_message(
            f"✅ Added {points} points to {user.mention}! New score: {new_score} 🏆",
            ephemeral=True,
        )

    except Exception as e:
        await interaction.response.send_message(
            f"❌ Error updating score: {e}", ephemeral=True
        )


@bot.tree.command(
    name="set_score",
    description="Set a user's score to a specific value (Admin only)",
)
@discord.app_commands.describe(
    user="The user to set score for", points="New score value"
)
@not_image_channel()
async def set_score_slash(
    interaction: discord.Interaction, user: discord.Member, points: int
):
    ...
    try:
        ...
        await interaction.response.send_message(
            f"✅ Set {user.mention}'s score to {points} points! 🏆",
            ephemeral=True,
        )

    except Exception as e:
        await interaction.response.send_message(
            f"❌ Error setting score: {e}", ephemeral=True
        )
...
async def reset_streak_slash(
    ...
    try:
        ...
        await interaction.response.send_message(
            f"❌ Error resetting streak: {e}", ephemeral=True
        )
...
async def reset_streaks():
    ...
    try:
        ...
        # Get users to reset
        c.execute(
            "SELECT user_id, username FROM user_streaks WHERE last_post_date < ? AND streak_days > 0",
            (yesterday,),
        )
        ...
        c.execute(
            "UPDATE user_streaks SET streak_days = 0 WHERE last_post_date < ?",
            (yesterday,),
        )
        ...
        if users_to_reset:
            print(
                f"Reset streaks for {len(users_to_reset)} users at {datetime.datetime.now()}"
            )
        else:
...
if __name__ == "__main__":
    ...
    if not bot_token:
        ...
        print(
            "Please set the DISCORD_BOT_TOKEN environment variable in Railway."
        )
        exit(1)
